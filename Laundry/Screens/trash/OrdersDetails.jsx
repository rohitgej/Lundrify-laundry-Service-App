...
const TrackOrder = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        const response = await fetch(`${DOMAIN_NAME}api/getcheckout`, requestOptions);
    ...
    const fetchPaymentMethods = async () => {
      ...
      try {
        const response = await fetch(`${DOMAIN_NAME}api/viewtransactionmethod`, requestOptions);
        const result = await response.json();
  ...
  return (
    <View style={styles.container}>
      <ScrollView horizontal contentContainerStyle={styles.statusContainer} showsHorizontalScrollIndicator={false}>
        {[
          { key:'1', stage: 'Ordered', icon: 'done' },
          { key:'2', stage: 'Pick up', icon: 'local-shipping' },
          { key:'3', stage: 'Progress', icon: 'autorenew' },
          { key:'4', stage: 'Out of Delivery', icon: 'local-shipping' },
          { key:'5', stage: 'Completed', icon: 'check-circle' },
        ].map((item, index) => (
          <View key={index} style={styles.stageContainer}>
            <View style={styles.iconContainer}>
              <View style={[styles.circle, item.key === String(deliveryStatus) && styles.activeCircle]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <TouchableOpacity style={styles.orderDetails} onPress={toggleExpand}>
          <View style={styles.orderCard}>
            ...
            <View style={styles.textContainer}>
              <Text style={styles.orderId}>Order id: {orderDetails.Customer_OrderNumber}</Text>
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={styles.verifyButton} onPress={() => navigation.navigate('Map',{order_id})}>
